...
class Trip
  ...
  def distance_from_origin
    current_point.distance_from_origin
  end

  def first_point_visited_twice
    visited_points = Set(Point).new

    each_visited_point do |point|
      if visited_points.includes?(point)
        return point
      else
  ...
  private def each_visited_point
    yield @transitions.first.not_nil!.first.point

    @transitions
      .map { |start_state, instruction, end_state| {start_state.point, end_state.point} }
      .select { |start_point, end_point| start_point != end_point }
      .each { |start_point, end_point| yield end_point }
  end
end
...
describe "Day 1 Part 2" do
  describe "first example" do
    trip = Trip.new(Document.new("R8, R4, R4, R8"))

    it "determines first location visited twice" do
      trip.first_point_visited_twice.should eq(Point.new(4, 0))
    end
  end
  ...
  describe "challenge example" do
    ...
    it "calculates distance to first location visited twice" do
      trip.first_point_visited_twice.not_nil!.distance_from_origin.should eq(126)
    end
  end
end
